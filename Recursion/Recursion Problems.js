...
const split = function (string, splitter) {
	let idx = string.indexOf(splitter);
...
const mazeSolver = function (maze, positions = [[0, 0]], visited = [[0, 0]]) {
	let num_rows = maze.length - 1;
	let num_columns = maze[0].length - 1;
...
function getAnagrams(string) {
	...
	for (var i = 0; i < string.length; i++) {
		...
		var otherChars = string.substring(0, i) + string.substring(i + 1);

		var innerPermutations = getAnagrams(otherChars);
