let countRibbonPieces = function (lengths, total) {
	function solveRecursive(lengths, total, currentIndex) {
		...
		let set2 = solveRecursive(lengths, total, currentIndex + 1);
		return Math.max(set1, set2);
	}

	let result = solveRecursive(lengths, total, 0);
	return result == Number.MIN_VALUE ? -1 : result;
};

let countRibbonPiecesDp = function (lengths, total) {
