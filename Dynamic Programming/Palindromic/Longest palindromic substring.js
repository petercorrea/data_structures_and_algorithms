...
let findLPSLengthMemo = function (string) {
	let memo = [];
	...
	function solveRecursive(string, startIdx, endIdx) {
		...
		if (typeof memo[startIdx][endIdx] == "undefined") {
			...
			if (string[startIdx] == string[endIdx]) {
				let remainingStringLength = endIdx - startIdx - 1;
...
let findLPSLengthTab = function (string) {
	let n = string.length;
...
console.log(`Length of LPS ---> ${findLPSLengthTab("abdbca")}`);
console.log(`Length of LPS ---> ${findLPSLengthTab("cddpd")}`);
console.log(`Length of LPS ---> ${findLPSLengthTab("pqr")}`);
